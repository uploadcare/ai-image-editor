...
vi.mock('@uploadcare/ai-enhancer', () => {
  throw new Error('@uploadcare/ai-enhancer must never be evaluated during SSR');
});
...
describe('SSR (bare Node, no DOM globals)', () => {
  ...
  it('renderToString renders nothing by default and does not throw', () => {
    const html = renderToString(<AiEnhancer pubkey="test-pubkey" />);
    expect(html).toBe('');
  });
  ...
  it('renderToString renders the fallback, never the custom element', () => {
    ...
    expect(html).not.toContain('uc-ai-enhancer');
  });
  ...
  it('preloadAiEnhancer is a no-op on the server', () => {
    expect(() => preloadAiEnhancer()).not.toThrow();
  });

  it('leaves the environment untouched after rendering', () => {
    renderToString(<AiEnhancer pubkey="test-pubkey" />);
    expect(typeof globalThis.HTMLElement).toBe('undefined');
    expect(typeof globalThis.customElements).toBe('undefined');
  });
});
