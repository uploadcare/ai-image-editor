...
describe('createMockBflProvider', () => {
  ...
  it('honours aspectRatio by fitting to the longEdge', async () => {
    const provider = createMockBflProvider({ ...FAST_LATENCY, longEdge: 600 });
    const wide = await provider.generate({ prompt: 'x', capability: 'generate', aspectRatio: [16, 9] });
    expect(wide.url).toMatch(/\/600\/338$/);
    const tall = await provider.generate({ prompt: 'x', capability: 'generate', aspectRatio: [9, 16] });
    expect(tall.url).toMatch(/\/338\/600$/);
    const square = await provider.generate({ prompt: 'x', capability: 'generate', aspectRatio: [1, 1] });
    expect(square.url).toMatch(/\/600\/600$/);
  });

  it('falls back to width/height when no aspectRatio is given', async () => {
    const provider = createMockBflProvider({ ...FAST_LATENCY, width: 320, height: 240 });
    const result = await provider.generate({ prompt: 'x', capability: 'generate' });
    expect(result.url).toMatch(/\/320\/240$/);
  });
