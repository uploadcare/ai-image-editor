...
| `metadata` | — | `Metadata` | Property only. Key/value metadata (`Record<string, string>`) attached to the result, for both generate and edit, e.g. `{ source: 'ai-enhancer' }`. Same shape as the file uploader's `metadata` config. |
