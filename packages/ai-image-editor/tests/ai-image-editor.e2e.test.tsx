...
import { cleanup, delay } from './test-renderer';

let UcAiImageEditorCtor: CustomElementConstructor;

beforeAll(async () => {
  // Importing the module registers <uc-ai-image-editor> and all sub-elements.
  const mod = await import('../src/index');
  UcAiImageEditorCtor = mod.UcAiImageEditor;
});
...
function mount(attrs: Record<string, string> = {}): UcAiImageEditorType {
...
function typePrompt(el: UcAiImageEditorType, value: string): void {
  const input = el.shadowRoot!.querySelector('uc-ai-prompt-row')!.shadowRoot!.querySelector('textarea')!;
  input.value = value;
  input.dispatchEvent(new Event('input', { bubbles: true }));
}

function primaryBtn(el: UcAiImageEditorType): HTMLButtonElement {
...
function clickPrimary(el: UcAiImageEditorType): void {
  primaryBtn(el).click();
}

/** The prompt row's send button triggers generation. */
function clickSend(el: UcAiImageEditorType): void {
...
const historyEl = (el: UcAiImageEditorType) =>
  el.shadowRoot!.querySelector('uc-ai-history') as (HTMLElement & { entries: unknown[] }) | null;

const canvasUrl = (el: UcAiImageEditorType): string | null =>
...
const editorMode = (el: UcAiImageEditorType): string =>
