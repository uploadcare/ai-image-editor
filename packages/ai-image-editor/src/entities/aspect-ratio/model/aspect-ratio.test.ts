...
describe('labelKeyForRatio', () => {
  it('returns the matching key for known ratios', () => {
    expect(labelKeyForRatio([1, 1])).toBe('ai-image-editor-aspect-square');
    expect(labelKeyForRatio([16, 9])).toBe('ai-image-editor-aspect-widescreen');
    expect(labelKeyForRatio([9, 16])).toBe('ai-image-editor-aspect-vertical');
  });
...
describe('toAspectRatioOption', () => {
  it('packs the ratio with its label key', () => {
    expect(toAspectRatioOption([1, 1])).toEqual({ value: [1, 1], labelKey: 'ai-image-editor-aspect-square' });
