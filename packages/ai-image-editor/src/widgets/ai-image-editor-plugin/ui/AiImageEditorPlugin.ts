...
import { type AiImageEditorLocale, enLocale, LOCALE_LOADERS } from '../../../shared/i18n';
import { ICON_EDIT_AI, ICON_GENERATE } from '../../../shared/ui/icons';
import type { DoneDetail, UcAiImageEditor } from '../../ai-editor';
...
const AI_IMAGE_EDITOR_ID = 'ai-image-editor';
...
const ACTIVITY_SIZE_STYLE_ID = 'uc-ai-image-editor-activity-size';
 ...
 * global rule matching the dialog that contains the active `ai-image-editor` activity.
...
function ensureActivityModalSize(): void {
  ...
  style.textContent = [
    `[uc-modal]:not(#\\#) > dialog:has([activity="${AI_IMAGE_EDITOR_ID}"][active]) {`,
    '  --uc-dialog-max-width: 1440px;',
    '  --uc-dialog-max-height: 900px;',
    '  width: 100%;',
    '  height: 100%;',
    '}',
  ].join('\n');
 ...
 * `setCurrentActivity(AI_IMAGE_EDITOR_ID, params)`, the editor's locale keys so
...
declare module '@uploadcare/file-uploader' {
  interface CustomActivities {
    [AI_IMAGE_EDITOR_ID]: { params: AiEditorActivityParams };
  }
  interface CustomLocaleDefinition extends AiImageEditorLocale {}
...
export const AiImageEditorPlugin: UploaderPlugin = {
  id: AI_IMAGE_EDITOR_ID,
  setup: ({ pluginApi, uploaderApi }) => {
    ...
    const localeCache = new Map<string, AiImageEditorLocale>([['en', enLocale]]);
    const registeredLocales = new Set<string>(['en']);

    const loadLocale = async (localeName: string): Promise<AiImageEditorLocale | null> => {
    ...
    registry.registerSource({
      id: AI_IMAGE_EDITOR_ID,
      label: 'ai-image-editor-source-label',
      icon: 'ai-generate',
      onSelect: () => {
        // No source → the editor opens in generate mode.
        uploaderApi.setCurrentActivity(AI_IMAGE_EDITOR_ID, {});
    ...
    registry.registerFileAction({
      id: AI_IMAGE_EDITOR_ID,
      icon: 'ai-edit',
      label: 'ai-image-editor-file-action-label',
      ...
      onClick: (fileEntry) => {
        uploaderApi.setCurrentActivity(AI_IMAGE_EDITOR_ID, { sourceInternalId: fileEntry.internalId });
        uploaderApi.setModalState(true);
      },
    });
    ...
    registry.registerActivity({
      ...
      render: (host, activityParams) => {
        if (!config.get('pubkey')) {
          console.warn('[ai-image-editor] No `pubkey` configured; the AI editor is disabled.');
          return () => {};
        }

        const params = (activityParams ?? {}) as AiEditorActivityParams;
        const editor = document.createElement('uc-ai-image-editor') as UcAiImageEditor;
        ...
        const onDone = (e: Event) => {
          ...
          // sourced to the AI Image Editor.
          if (params.sourceInternalId) {
            try {
              ...
              uploaderApi.replaceFile(params.sourceInternalId, file, { source: AI_IMAGE_EDITOR_ID });
            } catch {
              // The source entry is gone (e.g. removed while editing) — fall back to adding.
              uploaderApi.addFileFromUploadcareFile(file, { source: AI_IMAGE_EDITOR_ID });
            }
          } else {
            uploaderApi.addFileFromUploadcareFile(file, { source: AI_IMAGE_EDITOR_ID });
          }
