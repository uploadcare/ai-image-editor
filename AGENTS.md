...
The guides in `packages/ai-image-editor/docs/` are mirrored to
...
- Guide pages map to `fern/pages/ai-image-editor/*.mdx` (`getting-started` →
  `quickstart`); `guide/plugin.md` maps to
  `fern/pages/file-uploader/ai-image-editor.mdx`; the props table of the
  *generated* `docs/api/components.md` (gitignored; produced from the
  `@property` JSDoc by `npm run docs:api`) feeds
  `fern/pages/ai-image-editor/options.mdx` (one heading per option, config
...
- This site (GitHub Pages) remains the home of the API reference
  ...
  `?utm_source=uploadcare-docs&utm_medium=docs&utm_campaign=ai-image-editor`.
- **CDN examples use the `%AI_IMAGE_EDITOR_VERSION%` placeholder**, substituted
  ...
  `ai-image-editor-version` Vite plugin in `docs/.vitepress/config.mts`). Never
